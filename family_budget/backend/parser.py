            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        f"MULTIPLE SECTIONS - VERY IMPORTANT:\n"
                        f"Israeli CC statements may have MULTIPLE sections (e.g., domestic + foreign transactions).\n"
                        f"The 'סך הכל' (total) line does NOT mean the end of the document!\n"
                        f"You MUST continue parsing ALL sections, even after seeing 'סך הכל'.\n"
                        f"Example structure:\n"
                        f"  [Domestic transactions...]\n"
                        f"  סך הכל\n"
                        f"  7937.21₪\n"
                        f"  [Foreign transactions - MUST also be parsed!]\n"
                        f"  סך הכל\n"
                        f"  2681.81₪\n\n"
                        f"FOREIGN CURRENCY TRANSACTIONS (עסקאות חו\"ל):\n"
                        f"The statement may include foreign/international transactions (like TESLA, SPOTIFY, etc.).\n"
                        f"These are REAL transactions that MUST be included. They typically appear:\n"
                        f"- AFTER the domestic 'סך הכל' line (in a separate section)\n"
                        f"- With notes like 'חיוב עסקת חו\"ל בש\"ח' or 'דחוי חודש'\n"
                        f"- With English merchant names (TESLA, SPOTIFY, etc.) and foreign cities (AMSTERDAM, STOCKHOLM)\n"
                        f"Extract these with the ₪ charge amount - they are already converted to ILS.\n\n"
                        f"SKIP THESE LINES (not transactions):\n"
                        f"- Summary/total lines ONLY: 'סך הכל' followed by a number like '7937.21₪' on the next line\n"
                        f"- Header rows: lines starting with 'תאריך עסקה שם בית העסק'\n"
                        f"- Empty or metadata lines\n\n"
                        f"RULES:\n"
                        ...
                        f"8. Each transaction should appear ONCE - do not create duplicates even if line appears twice\n"
